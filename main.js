...
// ╔══════════════════════════════════════════════════════════════════════════╗
//  API REST — servidor Express para la app móvil
...
//  GET  /api/pedidos             → lista pedidos (para la app PC)
...
function startApiServer () {
  ...
  apiServer = http.createServer((req, res) => {
    // CORS — permite peticiones desde Expo/React Native
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (rutaAretes) {
            ...
            for (const ext of ['.jpg', '.jpeg', '.png', '.webp']) {
              if (fs.existsSync(path.join(fotosDir, `${codigo}${ext}`))) {
                tieneFoto = true; break
              }
            }
          }
          // Verificar si existe STL
          let tieneStl = false
      ...
      if (req.method === 'POST' && route === '/api/pedidos') {
        bodyJSON().then(body => {
          ...
          `)
          const result = stmt.run(fecha, cliente, notas, JSON.stringify(items))
          // Notificar a la ventana Electron que llegó un pedido nuevo
...
// ╔══════════════════════════════════════════════════════════════════════════╗
//  IPC — PEDIDOS (para la página Pedidos en la app PC)
// ╚══════════════════════════════════════════════════════════════════════════╝
...
// ╔══════════════════════════════════════════════════════════════════════════╗
//  COLORES — lee de la tabla filamentos (DB compartida)
// ╚══════════════════════════════════════════════════════════════════════════╝
