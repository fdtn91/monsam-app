 ...
 *   pedidos      — pedidos recibidos desde la app móvil (monsam-app)
 *   modelos_nuevos — marcador hasta que el usuario lo vea (monsam-app)
 */
...
function initSchema (db) {
  db.exec(`
    ...
    -- ── Pedidos (monsam — recibidos desde app móvil) ─────────────────────────
    --   items: JSON array de { codigo, carpeta, color, color_hex, pares }
    --   estado: 'pendiente' | 'visto' | 'en_proceso' | 'listo' | 'entregado'
...
function migrateFromExcel (db, cfg) {
  ...
  if (countFil === 0 && cfg.rutaExcel) {
    ...
    if (wbE && wbE.Sheets['Filamentos']) {
      ...
      const ins = db.prepare(`
        ...
        VALUES (?,?,?,?,?,?,?)
      `)
      const many = db.transaction((data) => { for (const r of data) ins.run(r) })
      const data = rows.slice(dataStart).filter(r => String(r[0]).trim()).map(r => [
        String(r[0]).trim(), String(r[1]||'').trim(), String(r[2]||'PLA').trim(),
        Number(r[3])||0, Number(r[4])||1000, Number(r[5])||0, String(r[8]||'#888888').trim()
      ])
      many(data)
  ...
  if (countImp === 0 && cfg.rutaExcel) {
    ...
    if (wbE && wbE.Sheets['Impresiones']) {
      ...
      const ins = db.prepare(`
        INSERT INTO impresiones (fecha,descripcion,filamento,gramos_usados,tiempo,categoria,resultado,costo_material)
        VALUES (?,?,?,?,?,?,?,?)
      `)
      const many = db.transaction((data) => { for (const r of data) ins.run(r) })
      const data = rows.slice(dataStart).filter(r => r[0]||r[1]||r[2]).map(r => [
        String(r[0]||''), String(r[1]||''), String(r[2]||''),
        Number(r[3])||0, String(r[4]||''), String(r[5]||'General'), String(r[6]||''), Number(r[7])||0
      ])
      many(data)
  ...
  if (countInv === 0 && cfg.rutaExcelMonsan) {
    ...
    if (wbM && wbM.Sheets['Inventario']) {
      ...
      const many = db.transaction((data) => { for (const r of data) ins.run(r) })
      const data = rows.slice(dataStart).filter(r => String(r[0]).trim()).map(r => [
        String(r[0]).trim(), String(r[1]||''), String(r[2]||''),
        String(r[3]||''), Number(r[4])||0, Number(r[5])||0
      ])
      many(data)
  ...
  if (countCli === 0 && cfg.rutaExcelMonsan) {
    const wbM = readWB(cfg.rutaExcelMonsan)
    if (wbM) {
      const TIPO_SHEET = { directo:'Clientes_Directos', distribuidor:'Distribuidores', mayoreo:'Mayoreo' }
      const ins = db.prepare(`
      ...
      for (const [tipo, sheet] of Object.entries(TIPO_SHEET)) {
        ...
        const dataStart = rows[0] && String(rows[0][0]).toLowerCase().includes('nombre') ? 1 : 0
        const data = rows.slice(dataStart).filter(r => String(r[0]).trim()).map(r => [
          tipo, String(r[0]).trim(), String(r[1]||''), String(r[2]||''), String(r[3]||''),
          String(r[4]||''), String(r[5]||''), String(r[6]||''),
          Number(r[7])||0, Number(r[8])||0, String(r[9]||'')
        ])
        many(data)
  ...
  if (countCos === 0 && cfg.rutaExcelMonsan) {
    ...
    if (wbM && wbM.Sheets['Costos']) {
      ...
      `)
      const many = db.transaction((data) => { for (const r of data) ins.run(r) })
      const data = rows.slice(dataStart).filter(r => r[0]).map(r => [
        String(r[0]||''), String(r[1]||''), String(r[2]||''),
        Number(r[3])||0, Number(r[4])||0, Number(r[5])||0, Number(r[6])||0,
        Number(r[7])||0, Number(r[8])||0, Number(r[9])||0, Number(r[10])||0, String(r[11]||'')
      ])
      many(data)
